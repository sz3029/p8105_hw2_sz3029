...
`r mr_trash_wheel_df %>% dplyr::group_by(year) %>% select(year, weight_tons:homes_powered) %>% skimr::skim() %>% select(-skim_type, -complete_rate, -numeric.p0, -numeric.p25, -numeric.p75, -numeric.p100) %>% knitr::kable(col.names = c("Variable", "Year", "Missing", "Mean", "SD", "Median", "Histogram"))`
